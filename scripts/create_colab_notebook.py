    ...
    "cells": [
        ...
        },
        {
            ...
            "source": [
                ...
                "    \"\"\"Convert SRT to ASS format with Arabic styling. Resilient against CRLF, BOM, bidi marks, timestamp variations.\"\"\"\n",
                "    # Pre-clean: strip BOM, normalize newlines, purge invisible bidi & zero-width chars\n",
                ...
                "    #    style is embedded in the ASS header \u2014 no force_style needed)\n",
                "    sub_abs = os.path.abspath(staged_sub).replace('\\\\', '/')\n",
                "    sub_filter_path = sub_abs.replace(':', '\\\\:')\n",
                "    subtitles_filter = f\"scale=trunc(iw/2)*2:trunc(ih/2)*2,ass='{sub_filter_path}'\"\n",
                "\n",
                ...
                "        print(f\"[HARDSUB] NVENC hardsubbing error ({proc.returncode}): {err_snippet}. Attempting CPU fallback (libx264 faster / crf {cq_val})...\")\n",
                "        cmd_cpu = [\n",
                "            \"ffmpeg\", \"-y\",\n",
                "            \"-i\", ffmpeg_input,\n",
                "            \"-vf\", subtitles_filter,\n",
